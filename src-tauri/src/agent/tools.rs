...
fn registry() -> Vec<ToolEntry> {
    vec![
        ...
        ToolEntry {
            kind: ToolKind::Pause,
            schema: schema("spawn_subagent", "Delegate one focused task to a subagent and get its report back as the tool result. Without a `worker`, the subagent runs on your own model, read-only: use 'explorer' to map a subsystem or 'reviewer' to critique a file without spending your own context. Name a `worker` — a CLI agent installed on this machine (claude-code, codex, opencode, omp) or a hosted API provider with a key (anthropic, openai, openrouter, …) — to hand the task to it as a Run of its own; then the editing roles 'implementer' and 'tester' are allowed too. Name an editing role with no worker and the user is asked which worker to use. A worker edits inside an isolated Git worktree on its own branch, the user approves the dispatch first, and the report tells you the branch to look at. Prefer this over running `claude` or `codex` yourself in the shell.",
                serde_json::json!({
                    ...
                    "worker": { "type": "string", "enum": crate::agent::subagents::worker_ids(), "description": "Optional. The CLI agent or API provider to hand this task to, as its own Run in an isolated worktree. Omit on an editing role and the user picks one." },
                    "model": { "type": "string", "description": "Optional, only with a `worker`: the model the worker should use. Omit and the user is asked; skipping leaves a CLI its own default." }
                }),
                &["subagent", "task"]),
            run_read: None,
            run_write_preview: None,
