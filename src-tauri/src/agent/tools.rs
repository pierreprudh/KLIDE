...
fn registry() -> Vec<ToolEntry> {
    vec![
        ...
        ToolEntry {
            kind: ToolKind::Pause,
            schema: schema("spawn_subagent", "Delegate one focused task to a subagent and get its report back as the tool result. Without a `worker`, the subagent runs on your own model, read-only: use 'explorer' to map a subsystem or 'reviewer' to critique a file without spending your own context. Name a `worker` — a CLI agent installed on this machine (claude-code, codex, opencode, omp) or a hosted API provider with a key (anthropic, openai, openrouter, …) — to hand the task to it as a Run of its own; then the editing roles 'implementer' and 'tester' are allowed too. Leave `worker` and `model` out unless the user named them in this conversation: the user is asked on a card and their pick wins, so naming one yourself only pre-selects it. A worker edits inside an isolated Git worktree on its own branch, the user approves the dispatch first, and the report tells you the branch to look at. Prefer this over running `claude` or `codex` yourself in the shell.",
                serde_json::json!({
