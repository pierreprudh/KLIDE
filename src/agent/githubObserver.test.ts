...
import { githubObserverLabel, githubObserverDetail, githubObserverDuration, githubObserverNeedsRefresh, type GithubObserver } from "./githubObserver";
const run = { status: "completed", conclusion: "success", prState: "open" } as GithubObserver;
it("keeps completed CI fresh until its PR merges or closes", () => {
  expect(githubObserverNeedsRefresh({ ...run, prNumber: 119 })).toBe(true);
  expect(githubObserverNeedsRefresh({ ...run, prNumber: 119, prState: "merged" })).toBe(false);
  expect(githubObserverNeedsRefresh({ ...run, prNumber: 119, prState: "closed" })).toBe(false);
  expect(githubObserverNeedsRefresh({ ...run, prNumber: null })).toBe(false);
  expect(githubObserverNeedsRefresh({ ...run, prNumber: null, status: "in_progress" })).toBe(true);
});
